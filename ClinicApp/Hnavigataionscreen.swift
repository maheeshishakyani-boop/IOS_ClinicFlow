...
struct HNavigationScreen: View {
    ...
    // MARK: - Location Section (working Menu dropdown)
            ...
            HStack(alignment: .top, spacing: Spacing.md) {
                ...
                VStack(alignment: .leading, spacing: Spacing.sm) {
                    ...
                    VStack(spacing: 0) {
                        ...
                        // ── Menu Button ──
                        Menu {
                            ForEach(allDestinations, id: \.id) { dest in
                                Button {
    ...
    private func directionStep(n: Int, text: String, last: Bool) -> some View {
        HStack(alignment: .top, spacing: 14) {
            VStack(spacing: 0) {
                ZStack {
                    Circle()
                        .fill(last ? AppColor.successLight : AppColor.accentLight)
                        .frame(width: 36, height: 36)
                    if last {
            ...
            VStack(alignment: .leading, spacing: 3) {
                Text(last ? "Arrived" : "Step \(n)")
                    .font(.caption.weight(.semibold))
                    .foregroundStyle(last ? AppColor.success : AppColor.accent)
                Text(text)
                    .font(.subheadline)
                    .foregroundStyle(Color(.label))
                    .fixedSize(horizontal: false, vertical: true)
        ...
        .padding(.horizontal, Spacing.base)
        .padding(.top, 14)
        .padding(.bottom, last ? Spacing.base : Spacing.xs)
