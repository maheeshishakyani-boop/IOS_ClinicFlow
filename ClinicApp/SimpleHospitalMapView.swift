...
struct SimpleHospitalMapView: View {
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                GeometryReader { geo in
                    ZStack {
                        mapBackground(size: geo.size)
                        HospitalRouteOverlay(start: userPos, end: destPos, size: geo.size)
                        HospitalMapPin(pos: userPos, color: green,  icon: "location.circle.fill",
                                       label: "You are here", size: geo.size)
                        HospitalMapPin(pos: destPos, color: accent, icon: "heart.circle.fill",
                                       label: "Cardiology",   size: geo.size)
                    }
                    .clipShape(RoundedRectangle(cornerRadius: 20))
                    .shadow(color: .black.opacity(0.10), radius: 12, y: 5)
                }
        ...
        .background(Color(.systemGroupedBackground))
        // ── Large title left-aligned at rest, collapses to center on scroll ──
        .navigationTitle("Indoor Navigation")
        ...
        HStack(alignment: .top, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                Text(last ? "Arrived" : "Step \(n)")
                    .font(.caption.weight(.semibold))
                    .foregroundStyle(last ? green : accent)
                Text(text)
                    .font(.subheadline)
                    .foregroundStyle(Color(.label))
                    .fixedSize(horizontal: false, vertical: true)
